...
public class OrderAcceptOutboxAdvice implements ResponseBodyAdvice<Object> {
    private static final String METHOD_NAME = new String(new char[] {'a', 'c', 'c', 'e', 'p', 't'});

    private final NotificationOutboxService outboxService;

    public OrderAcceptOutboxAdvice(NotificationOutboxService outboxService) {
    ...
    public Object beforeBodyWrite(Object body, MethodParameter returnType, MediaType selectedContentType, Class<? extends HttpMessageConverter<?>> selectedConverterType, ServerHttpRequest request, ServerHttpResponse response) {
        if (body instanceof OrderResponse order) {
            outboxService.recordChefAccepted(order);
        }
        return body;
    }
}
