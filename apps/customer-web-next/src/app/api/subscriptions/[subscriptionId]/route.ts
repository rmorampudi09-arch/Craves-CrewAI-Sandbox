...
export async function GET(request: NextRequest, context: { params: Promise<{ subscriptionId: string }> }) {
  ...
  try {
    ...
    if (!upstream.ok) return NextResponse.json({ code: upstream.status === 401 ? "SESSION_EXPIRED" : upstream.status === 404 ? "SUBSCRIPTION_NOT_FOUND" : "SUBSCRIPTION_REQUEST_FAILED" }, { status: upstream.status });
