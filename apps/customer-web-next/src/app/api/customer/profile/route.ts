...
function failure(status: number, message = "Customer profile is unavailable.") {
  return NextResponse.json(
    {
      error: status === 401 ? "SESSION_REQUIRED" : "PROFILE_REQUEST_FAILED",
      message: status === 401 ? "Please sign in again." : message,
    },
    { status },
  );
}
...
export async function GET(request: NextRequest) {
  try {
    ...
    return profile
      ...
      : failure(502, "Customer profile response validation failed.");
  } catch (error) {
    ...
    return NextResponse.json(
      {
        ...
        message: "Invalid profile request origin.",
      },
      { status: 403 },
    );
  }
    ...
    return NextResponse.json(
      {
        ...
        message: "Enter a valid first name, last name and optional email.",
      },
      { status: 400 },
    );
  }
