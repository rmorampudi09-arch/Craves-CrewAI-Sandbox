...
export async function POST(request: NextRequest, context: { params: Promise<{ applicationId: string }> }) {
  ...
  try {
    const upstream = await authenticatedApiFetch(request, `/backoffice/chef-reviews/${applicationId}/approve`, { method: "POST" });
    const body = await upstream.json().catch(() => null);
    if (!upstream.ok) return NextResponse.json({ code: upstream.status === 401 ? "SESSION_EXPIRED" : upstream.status === 403 ? "ADMIN_ACCESS_REQUIRED" : "CHEF_APPROVAL_FAILED" }, { status: upstream.status });
    ...
    return NextResponse.json({ code: "CHEF_APPROVAL_UNAVAILABLE" }, { status: 503 });
  }
}
