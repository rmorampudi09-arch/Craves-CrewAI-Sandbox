    ...
    <section>
      <div className="rounded-[30px] bg-[#FFF8EC] p-7 text-slate-950">
        ...
        <p className="mt-3 text-sm text-slate-600">
          This shell does not grant roles. Every operation is re-authorized by its owning backend service.
        </p>
      </div>
      <div className="mt-6 grid gap-5 md:grid-cols-2 xl:grid-cols-3">
        {links.map(link => (
          <a key={link.href} href={link.href} className="rounded-[28px] border border-white/10 bg-white/5 p-6 text-white transition hover:bg-white/10">
