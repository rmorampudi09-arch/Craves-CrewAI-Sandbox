...
if ! az apim api show -g "$RG" --service-name "$APIM" --api-id "$API_ID" -o none 2>/dev/null; then
  echo "SUCCESS: Account intervention API is already absent."
  exit 0
fi

APPROVED=(get-admin-account-intervention-status post-admin-account-suspend post-admin-account-reactivate)
