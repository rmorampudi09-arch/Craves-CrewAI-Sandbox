...
API_ID_DEFAULT="craves-chef-kitchen-v1"
...
if (( ${#API_IDS[@]} == 0 )); then
  ...
  az apim api create -g "$RG" --service-name "$APIM" --api-id "$API_ID" --display-name "Craves Chef Kitchen API" --path "$API_PATH" --protocols https --subscription-required false -o none
else
  API_ID="${API_IDS[0]}"
...
put_operation(){
  ...
  printf '%s' "{\"properties\":{\"displayName\":\"$DISPLAY\",\"method\":\"$METHOD\",\"urlTemplate\":\"$TEMPLATE\",\"templateParameters\":[],\"responses\":[{\"statusCode\":200,\"description\":\"Chef kitchen response\"},{\"statusCode\":401,\"description\":\"Authentication required\"},{\"statusCode\":403,\"description\":\"Chef role required\"}]}}" >"$BODY"
...
put_operation "get-my-chef-kitchen" "GET" "/" "Get my chef kitchen"
put_operation "upsert-my-chef-kitchen" "PUT" "/" "Create or update my chef kitchen"
for ID in get-my-chef-kitchen upsert-my-chef-kitchen; do
  ...
  [[ "$POLICY" == *"$BACKEND"* && "$POLICY" == *"Authorization"* ]] || fail "Operation $ID verification failed"
done
echo "SUCCESS: Chef kitchen profile APIM operations configured on API $API_ID."
