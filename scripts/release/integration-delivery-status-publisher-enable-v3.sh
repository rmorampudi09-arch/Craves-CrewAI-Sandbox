...
MAX_ATTEMPTS=${CRAVES_PUBLISHER_ACTIVATION_MAX_ATTEMPTS:-150}
POLL_SECONDS=${CRAVES_PUBLISHER_ACTIVATION_POLL_SECONDS:-10}
...
[[ "$MAX_ATTEMPTS" =~ ^[1-9][0-9]*$ ]] || fail 'CRAVES_PUBLISHER_ACTIVATION_MAX_ATTEMPTS must be a positive integer.'
[[ "$POLL_SECONDS" =~ ^[1-9][0-9]*$ ]] || fail 'CRAVES_PUBLISHER_ACTIVATION_POLL_SECONDS must be a positive integer.'
...
wait_for_revision() {
  ...
  local attempt final latest ready running publisher revision_json
  local health provisioning revision_running replicas target_revision=''
  ...
  for attempt in $(seq 1 "$MAX_ATTEMPTS"); do
    ...
    if [[ -z "$target_revision" && -n "$latest" && "$latest" != "$previous_latest" ]]; then
      target_revision=$latest
      echo "Observed publisher activation revision: $target_revision" >&2
    fi

    if [[ -n "$target_revision" && "$latest" != "$target_revision" ]]; then
      echo "ERROR: latest revision changed unexpectedly while waiting. expected=$target_revision actual=$latest" >&2
      return 2
    fi
    ...
    if [[ -n "$latest" ]]; then
      ...
      if [[ -n "$revision_json" ]] && jq -e . >/dev/null 2>&1 <<<"$revision_json"; then
        health=$(jq -r '.properties.healthState // ""' <<<"$revision_json")
        provisioning=$(jq -r '.properties.provisioningState // ""' <<<"$revision_json")
        revision_running=$(jq -r '.properties.runningState // ""' <<<"$revision_json")
        replicas=$(jq -r '.properties.replicas // 0' <<<"$revision_json")
      fi
    fi

    echo "Attempt $attempt/$MAX_ATTEMPTS: latest=$latest ready=$ready health=${health:-<empty>} provisioning=${provisioning:-<empty>} revisionRunning=${revision_running:-<empty>} replicas=${replicas:-<unknown>} appRunning=$running statusPublisher=${publisher:-<empty>}" >&2

    if [[ "$provisioning" == 'Failed' || "$revision_running" == 'Failed' ]]; then
      echo "ERROR: activation revision entered a terminal failure state. provisioning=${provisioning:-<empty>} running=${revision_running:-<empty>}" >&2
      return 3
    fi
...
echo "Activation observation window: $MAX_ATTEMPTS attempts x $POLL_SECONDS seconds"
...
if ! FINAL=$(wait_for_revision "$PREVIOUS_LATEST" true false); then
  ...
  fail "Publisher activation revision did not become healthy within the observation window ($((MAX_ATTEMPTS * POLL_SECONDS)) seconds), or entered a terminal failure state."
fi
